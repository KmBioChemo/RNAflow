...
`figure1.pdf`), then re-render:
`pandoc paper/paper.md --citeproc --bibliography=paper.bib -o paper/paper.pdf`.
The placeholder schematic currently at that path (from `paper/make_overview.R`)
is only a stand-in.
