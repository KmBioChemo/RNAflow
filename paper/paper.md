...
expected glucocorticoid response (Additional file 1: Figure S1).
<!-- TODO: from paper/make_supp_activity.R (it prints the top pathways), name the
     leading PROGENy pathway(s) and their direction here, e.g. "reduced NF-kB and
     TNFalpha activity, consistent with the anti-inflammatory action of
     dexamethasone". Do not state specific pathways/scores until confirmed by the
     script's own output. -->
...
inferred activity. Generated by `paper/make_supp_activity.R` (the PROGENy network
is fetched from OmniPath once and cached locally, so the figure re-runs offline).
*TODO: after running the script, confirm the caption's biological claim against
the top pathways it prints.* (`paper/figures/figureS1_activity.pdf`.)
